...
class TwoStageInferencePipeline:
    def __init__(self, person_model_path, ppe_model_path, classes_file="classes.txt"):
    ...
    def _load_classes(self, classes_file):
        if not os.path.exists(classes_file):
            logging.warning(f"{classes_file} not found. Using default index mapping labels.")
            return []
    ...
    def process_image(self, img_path, output_dir, conf_threshold=0.25):
        ...
        # verbose=False suppresses dense terminal printing for clean logs
        ...
        for person_box in person_results.boxes:
            # Check if class matches person (assuming person index is 0)
            ...
            # Draw the Person Bounding Box directly using OpenCV
            cv2.rectangle(frame, (p_xmin, p_ymin), (p_xmax, p_ymax), self.colors['person'], 2)
            label = f"person {p_conf:.2f}"
            cv2.putText(frame, label, (p_xmin, max(p_ymin - 10, 10)), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.colors['person'], 2)

            # Extract image crop safely ensuring coordinates remain within frame limits
            h, w, _ = frame.shape
            p_xmin_safe, p_ymin_safe = max(0, p_xmin), max(0, p_ymin)
            p_xmax_safe, p_ymax_safe = min(w, p_xmax), min(h, p_ymax)
            
            person_crop = frame[p_ymin_safe:p_ymax_safe, p_xmin_safe:p_xmax_safe]
            ...
            for ppe_box in ppe_results.boxes:
                ...
                ppe_cls_name = self.get_class_name(ppe_cls_id)
                ...
                # Draw the mapped PPE box onto the main full canvas using OpenCV
                cv2.rectangle(frame, (global_ppe_xmin, global_ppe_ymin), 
                              (global_ppe_xmax, global_ppe_ymax), color, 2)
                
                ppe_label = f"{ppe_cls_name} {ppe_conf:.2f}"
                cv2.putText(frame, ppe_label, (global_ppe_xmin, max(global_ppe_ymin - 10, 10)), 
    ...
    def run_directory_pipeline(self, input_dir, output_dir):
        ...
        # Handle various generic image extensions cleanly
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Two-Stage Vision Pipeline: Person & Personal Protective Equipment Detection Engine.")
    ...
    parser.add_argument("--classes_file", type=str, default="classes.txt", help="Path configuration referencing classes text mapper.")
